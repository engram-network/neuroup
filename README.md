...
[![build](https://github.com/engram-network/tokios/actions/workflows/test.yml/badge.svg)](https://github.com/a16z/tokios/actions/workflows/test.yml) [![license: MIT](https://img.shields.io/badge/license-MIT-brightgreen.svg)](https://opensource.org/licenses/MIT) [![chat](https://img.shields.io/badge/chat-telegram-blue)](https://t.me/+IntDY_gZJSRkNTJj)

Tokios is a fully trustless, efficient, and portable Ethereum light client written in Rust.

Tokios converts an untrusted centralized RPC endpoint into a safe unmanipulable local RPC for its users. It syncs in seconds, requires no storage, and is lightweight enough to run on mobile devices.

The entire size of Tokios's binary is 5.3Mb and should be easy to compile into WebAssembly. This makes it a perfect target to embed directly inside wallets and dapps.

## Installing

First install `tokios`, Tokios's installer:

```
curl https://raw.githubusercontent.com/engram-network/tokios/master/tokios/install | bash
```

To install Tokios, run `tokios`.

## Testing

To ensure that Tokios works as expected, we have a comprehensive test suite that you can run. Before running the tests, make sure to create a `.env` file in the root of the project directory. You can copy the contents of the `.env.example` file and fill in your own secrets.
...
To run Tokios, run the below command, replacing `$ETH_RPC_URL` with an RPC provider URL such as Alchemy or Infura:

```
tokios --execution-rpc $ETH_RPC_URL
```
...
Tokios will now run a local RPC server at `http://127.0.0.1:8545`.

Tokios provides examples in the [`examples/`](./examples/) directory. To run an example, you can execute `cargo run -p tokios --example <example_name>` from inside the tokios repository.

Tokios also provides documentation of its supported RPC methods in the [rpc.md](./rpc.md) file.

### Warning

Tokios is still experimental software. While we hope you try it out, we do not suggest adding it as your main RPC in wallets yet. Sending high-value transactions from a wallet connected to Tokios is discouraged.
...
`--data-dir` or `-d` sets the directory that Tokios should use to store cached weak subjectivity checkpoints in. Each network only stores the latest checkpoint, which is just 32 bytes.

`--fallback` or `-f` sets the checkpoint fallback url (a string). This is only used if the checkpoint provided by the `--checkpoint` flag is too outdated for Tokios to use to sync.
If none is provided and the `--load-external-fallback` flag is not set, Tokios will error.
For example, you can specify the fallback like so: `tokios --fallback "https://sync-mainnet.beaconcha.in"` (or using shorthand like so: `tokios -f "https://sync-mainnet.beaconcha.in"`)

`--load-external-fallback` or `-l` enables weak subjectivity checkpoint fallback (no value needed).
For example, say you set a checkpoint value that is too outdated and Tokios cannot sync to it.
If this flag is set, Tokios will query all network apis in the community-maintained list
...
This can be run like so: `tokios --load-external-fallback` (or `tokios -l` with the shorthand).

`--strict-checkpoint-age` or `-s` enables strict checkpoint age checking. If the checkpoint is over two weeks old and this flag is enabled, Tokios will error. Without this flag, Tokios will instead surface a warning to the user and continue. If the checkpoint is greater than two weeks old, there are theoretical attacks that can cause Tokios and over light clients to sync incorrectly. These attacks are complex and expensive, so Tokios disables this by default.

`--help` or `-h` prints the help message.

### Configuration Files

All configuration options can be set on a per-network level in `~/.Tokios/tokios.toml`. Here is an example config file:
...
### Using Tokios as a Library

Tokios can be imported into any Rust project.

```rust
use std::{str::FromStr, env};

use tokios::{client::ClientBuilder, config::networks::Network, types::BlockTag};
...
```rust
use eyre::Result;
use tokios::config::{checkpoints, networks};
...
To learn more about [Tokios](https://github.com/engram-network/tokios) benchmarking and to view benchmark flamegraphs, view the [benchmark readme](./benches/README.md).


## Contributing

All contributions to Tokios are welcome. Before opening a PR, please submit an issue detailing the bug or feature. When opening a PR, please ensure that your contribution builds, has been linted with `cargo fmt`, and contains tests when applicable.

## Telegram

If you are having trouble with Tokios or are considering contributing, feel free to join our telegram [here](https://t.me/+IntDY_gZJSRkNTJj).
