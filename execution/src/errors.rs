...
pub enum ExecutionError {
    ...
    IncorrectRpcNetwork(),
    #[error("Invalid base gas fee tokios {0} vs rpc endpoint {1} at block {2}")]
    InvalidBaseGaseFee(U256, U256, u64),
    #[error("Invalid gas used ratio of tokios {0} vs rpc endpoint {1} at block {2}")]
    InvalidGasUsedRatio(f64, f64, u64),
    #[error("Block {0} not found")]
    BlockNotFoundError(u64),
    #[error("tokios Execution Payload is empty")]
    EmptyExecutionPayload(),
    #[error("User query for block {0} but tokios oldest block is {1}")]
